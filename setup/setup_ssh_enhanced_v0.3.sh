...
# Enhanced GitHub SSH Setup v0.3 - macOS Optimized
# Features: No jq dependency, enhanced visuals, better verification feedback, emoji emphasis

# Source the external spinner library
...
if [ -f "$SPINNER_LIB" ]; then
    ...
    echo "Warning: Spinner library not found at $SPINNER_LIB" >&2
fi
...
# Enhanced verification function with visual feedback
verify_ssh_connection() {
    local account_clean="$1"
    local account="$2"
    
    echo -e "\n${EMOJI_MAG}${CYAN} Testing SSH connection... ${YELLOW}(This may take up to 30 seconds)${NC}"
    echo -e "${WHITE}Progress:${NC} ${EMOJI_CLOCK} Connecting to GitHub..."
    
    # Start verification in background with timeout
    (
        timeout 30 ssh -T git@github-$account_clean 2>&1 | grep -q "successfully authenticated"
        echo $? > /tmp/ssh_test_result
    ) &
    ...
    if [ -f /tmp/ssh_test_result ] && [ "$(cat /tmp/ssh_test_result)" = "0" ]; then
        ...
        echo -e "  ${EMOJI_ARROW} Did you complete all 4 steps above?"
        echo -e "  ${EMOJI_ARROW} Did you log OUT of GitHub completely?"
        echo -e "  ${EMOJI_ARROW} Check if the key was added correctly in GitHub settings"
        echo -e "  ${EMOJI_ARROW} Try running: ${CYAN}ssh -T git@github-$account_clean${NC}"
        return 1
    fi
    rm -f /tmp/ssh_test_result
}

# Print header
echo -e "\n${CYAN}${EMOJI_ROCKET} Enhanced GitHub SSH Setup v0.3 ${EMOJI_FIRE}${NC}"
echo -e "${WHITE}================================${NC}"
...
for account in "${accounts[@]}"; do
    ...
    if [ ! -f "$key_private" ]; then
        ...
        echo -e "${EMOJI_STAR}${CYAN} Step 1:${NC} ${WHITE}Open GitHub.com in your browser${NC}"
        echo -e "         ${YELLOW}(Browser will open automatically in 3 seconds)${NC}"
        
        echo -e "\n${EMOJI_STAR}${CYAN} Step 2:${极狐} Go to Settings → SSH and GPG keys → New SSH key${NC}"
        
        echo -e "\n${EMOJI_STAR}${CYAN} Step 3:${NC} ${WHITE}Paste the key below and give it a title:${NC}"
        echo -e "${YELLOW}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${NC}"
        cat "$key_public"
        echo -e "${YELLOW}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${NC}"
        
        echo -e "\n${EMOJI_FIRE}${CYAN} Step 4:${NC} ${WHITE}IMPORTANT: Log OUT of GitHub completely!${NC}"
        ...
        # Enhanced verification with timeout and visual feedback
        verify_ssh_connection "$account_clean" "$account"
    fi
done
